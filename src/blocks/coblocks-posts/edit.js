...
const styleOptions = [
	{
		...
		label: __( 'Stacked', 'pbrocks-block-starter' ),
	...
	},
	{
		name: 'horizontal',
		/* translators: block style */
		label: __( 'Horizontal', 'pbrocks-block-starter' ),
...
const PostsEdit = ( props ) => {
	...
	const editToolbarControls = [
		{
			icon: edit,
			title: __( 'Edit RSS URL', 'pbrocks-block-starter' ),
	...
	const toolbarControls = [ {
		...
		title: __( 'Image on left', 'pbrocks-block-starter' ),
		isActive: listPosition === 'left',
		onClick: () => setAttributes( { listPosition: 'left' } ),
	}, {
		icon: <Icon icon={ pullRight } />,
		title: __( 'Image on right', 'pbrocks-block-starter' ),
	...
	if ( ! hasPosts && postFeedType === 'internal' ) {
		return (
			<>
				...
				<Placeholder
					icon={ <Icon icon={ icon } /> }
					label={ __( 'Blog Posts', 'pbrocks-block-starter' ) }
				>
					{ ! Array.isArray( latestPosts )
						? <Spinner />
						: <>
							{ __( 'No posts found. Start publishing or add posts from an RSS feed.', 'pbrocks-block-starter' ) }
							<Button
								className="components-placeholder__cancel-button"
								title={ __( 'Retrieve an External Feed', 'pbrocks-block-starter' ) }
								isSecondary
								onClick={ () => {
									setAttributes( { postFeedType: 'external' } );
								} }
							>
								{ __( 'Use External Feed', 'pbrocks-block-starter' ) }
							</Button>
						</>
					}
				</Placeholder>
	...
	if ( editing && postFeedType === 'external' ) {
		return (
			<>
				...
				<Placeholder
					icon={ <Icon icon={ icon } /> }
					label={ __( 'RSS Feed', 'pbrocks-block-starter' ) }
					instructions={ __( 'RSS URLs are generally located at the /feed/ directory of a site.', 'pbrocks-block-starter' ) }
				>
					<form onSubmit={ onSubmitURL }>
						<TextControl
							placeholder={ __( 'https://example.com/feed…', 'pbrocks-block-starter' ) }
							value={ externalRssUrl }
							onChange={ ( value ) => setAttributes( { externalRssUrl: value } ) }
							className={ 'components-placeholder__input' }
						/>
						<Button type="submit" disabled={ ! externalRssUrl }>
							{ __( 'Use URL', 'pbrocks-block-starter' ) }
						</Button>
					</form>
				</Placeholder>
			</>
	...
	return (
		<>
			...
			{ postFeedType === 'external' &&
				<Disabled>
					<ServerSideRender
						block="pbrocks-block-starter/coblocks-posts"
						attributes={ attributes }
					/>
				</Disabled>
			}
			{ postFeedType === 'internal' &&
				...
				<div className={ className }>
					<GutterWrapper { ...attributes } condition={ attributes.columns >= 2 }>
						<div className={ classnames( 'wp-block-pbrocks-block-starter-coblocks-posts__inner', {
							'has-columns': columns,
							...
							{ displayPosts.map( ( post, i ) => {
								...
								const contentClasses = classnames( 'wp-block-pbrocks-block-starter-coblocks-posts__content', {
								...
								return (
									<div key={ i } className="wp-block-pbrocks-block-starter-coblocks-posts__item">
										{ featuredImageUrl &&
											<div className="wp-block-pbrocks-block-starter-coblocks-posts__image">
										...
										<div className={ contentClasses }>
											{ isStackedStyle && displayPostDate && post.date_gmt &&
												<time dateTime={ format( 'c', post.date_gmt ) } className="wp-block-pbrocks-block-starter-coblocks-posts__date">
											...
											<Disabled>
												<a href={ post.link } target="_blank" rel="noreferrer noopener" alt={ titleTrimmed }>
													{ titleTrimmed ? (
														...
														: __( '(no title)', 'pbrocks-block-starter' )
													}
												</a>
											</Disabled>
											...
											{ displayPostContent &&
												<div className="wp-block-pbrocks-block-starter-coblocks-posts__excerpt">
													<RawHTML
														key="html"
													>
											...
											{ displayPostLink &&
												<RichText
													tagName="a"
													className="wp-block-pbrocks-block-starter-coblocks-posts__more-link"
													onChange={ ( newPostLink ) => setAttributes( { postLink: newPostLink } ) }
													value={ postLink }
													placeholder={ __( 'Read more', 'pbrocks-block-starter' ) }
													multiline={ false }
